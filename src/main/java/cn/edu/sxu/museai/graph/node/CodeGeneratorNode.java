...
public class CodeGeneratorNode {
    ...
    public static AsyncNodeAction<MessagesState<String>> create() {
        return node_async(state -> {
            ...
            Long appId = 0L;
            // 调用流式代码生成
            Flux<String> codeStream = codeGeneratorFacade.generateCodeAndSaveStreaming(userMessage, generationType, appId, 0L);
            // 同步等待流式输出完成
            codeStream.blockLast(Duration.ofMinutes(10)); // 最多等待 10 分钟
            // 根据类型设置生成目录
            String generatedCodeDir = String.format("%s/%s/%s", AppConstant.CODE_BATH_PATH, generationType.getValue(), appId);
