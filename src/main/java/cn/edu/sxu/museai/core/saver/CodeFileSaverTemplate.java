...
public abstract class CodeFileSaverTemplate<T> {
    private static final String basePath = AppConstant.CODE_BATH_PATH;

    public File saveCode(T codeResult, Long appId) {
        // 1. 校验输入
        validateInput(codeResult);
        // 2. 获取路径
        CodeGenTypeEnum codeGenTypeEnum = codeGenTypeEnum();
        String basePath = filePath(codeGenTypeEnum, appId);
        // 3. 保存文件
    ...
    private String filePath(CodeGenTypeEnum codeGenTypeEnum, Long appId) {
        String path = basePath + "/" + codeGenTypeEnum.getValue() + "/" + appId;
        FileUtil.mkdir(path);
        return path;
    }
